...
from dfm.api.esri import LoadUSWindForecastData
...
def mock_adapter():
    ...
    adapter.params = LoadUSWindForecastData(
...
async def test_cache_file_name_basic(caching_iterator):
    ...
    expected = "1234567890000_national.json"
...
async def test_cache_file_name_for_each_layer(
    ...
    expected_names = {
        "national": "1234567890000_national.json",
        "regional": "1234567890000_regional.json",
        "state": "1234567890000_state.json",
        "county": "1234567890000_county.json",
        "district": "1234567890000_district.json",
        "block_group": "1234567890000_block_group.json",
        "city": "1234567890000_city.json",
    }

    for layer, expected_name in expected_names.items():
        # Configure adapter for this layer
        FunctionCall.set_allow_outside_block()
        mock_adapter.params = LoadUSWindForecastData(
...
@pytest.mark.asyncio
async def test_cache_file_name_with_time_filter(
    mock_adapter, mock_filesystem, mock_config
):
    """Test cache file name generation with time filter."""
    FunctionCall.set_allow_outside_block()
    mock_adapter.params = LoadUSWindForecastData(
        layer="national",
        return_geojson=True,
        return_meta_data=True,
        time_filter=["2025-04-01T00:00", "2025-04-02T00:00"],
    )
    FunctionCall.unset_allow_outside_block()

    with patch("fsspec.filesystem", return_value=mock_filesystem):
        iterator = GeoJsonFileCachingIterator(mock_adapter, mock_config)
        expected = "1234567890000_national_2025-04-01T00-00_2025-04-02T00-00.json"
        assert iterator.get_cache_file_name() == expected


@pytest.mark.asyncio
async def test_cache_file_name_with_time_filter_for_each_layer(
    mock_adapter, mock_filesystem, mock_config
):
    """Test file name generation for each layer value with time filter."""
    time_filter = ["2025-04-01T00:00", "2025-04-02T00:00"]
    expected_names = {
        "national": "1234567890000_national_2025-04-01T00-00_2025-04-02T00-00.json",
        "regional": "1234567890000_regional_2025-04-01T00-00_2025-04-02T00-00.json",
        "state": "1234567890000_state_2025-04-01T00-00_2025-04-02T00-00.json",
        "county": "1234567890000_county_2025-04-01T00-00_2025-04-02T00-00.json",
        "district": "1234567890000_district_2025-04-01T00-00_2025-04-02T00-00.json",
        "block_group": "1234567890000_block_group_2025-04-01T00-00_2025-04-02T00-00.json",
        "city": "1234567890000_city_2025-04-01T00-00_2025-04-02T00-00.json",
    }

    for layer, expected_name in expected_names.items():
        # Configure adapter for this layer with time filter
        FunctionCall.set_allow_outside_block()
        mock_adapter.params = LoadUSWindForecastData(
            layer=layer,
            return_geojson=True,
...
async def test_load_values_from_cache_metadata_only(caching_iterator):
    ...
    metadata = {"timestamp": "2025-04-01T00:00", "features": "10"}
...
async def test_load_values_from_cache_geojson_only(caching_iterator, mock_config):
    ...
    FunctionCall.set_allow_outside_block()
    caching_iterator.adapter.params = LoadUSWindForecastData(
