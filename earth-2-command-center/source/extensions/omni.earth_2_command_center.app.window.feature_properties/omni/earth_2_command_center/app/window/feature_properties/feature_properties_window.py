...
class FeaturePropertyModel(ui.AbstractValueModel):
    ...
    def _on_feature_change(self, event):
        ...
        if change['id'] == features_api_module.FeatureChange.PROPERTY_CHANGE['id']:
            ...
            if property_name == self._property_name:
                # we need to notify the ui elements of this change
                self.set_value(self._get(), force_notify=True)
                return True
        return False
    ...
    def set_value(self, value, force_notify=False):
        value = self._to_native_type(value)
        if value != self._get():
        ...
        with frame:
            with ui.VStack(
                ...
                if feature.type == features_api_module.Image.feature_type:
                        ...
                        with frame:
                            def add_remapping_entry(label, name, default, min_value, max_value):
                                with ui.HStack(height = line_height):
                                    ...
                                    ui.FloatDrag(model, min=min_value, max=max_value, step=(max_value-min_value)/1000, enabled = True)
                    ...
                    with ui.HStack(height = line_height):
                        ui.Label('Loop', width = label_width)
                        model = ui.SimpleBoolModel(feature.loop)
                        self._models.append(model)
                        def adjust_loop(model):
                            feature.loop = model.get_value_as_bool()
                        model.add_value_changed_fn(adjust_loop)
                    ...
                    def adjust_enabled(target, invert, model):
                        target.enabled = model.get_value_as_bool() if not invert else not model.get_value_as_bool()
                    follow_camera_model = FeaturePropertyModel(feature, 'follow_camera')

                    with ui.HStack(height = line_height):
                        ui.Label('Diurnal Motion', width = label_width)
                        model = FeaturePropertyModel(feature, 'diurnal_motion')
                        self._property_models.append(model)
                        field = ui.CheckBox(model, enabled = not feature.follow_camera)
                        follow_camera_model.add_value_changed_fn(partial(adjust_enabled, field, True))
                    with ui.HStack(height = line_height):
                        ...
                        follow_camera_model.add_value_changed_fn(partial(adjust_enabled, field, True))
                    with ui.HStack(height = line_height):
                        ui.Label('Follow Camera', width = label_width)
                        self._property_models.append(follow_camera_model)
                        field = ui.CheckBox(follow_camera_model, enabled=True)
                    with ui.HStack(height = line_height):
