    ...
    def update_transform(self, meta_data: dict[str, Any],
        ...
        if lat and lon:
            ...
            if not is_full_globe:
                affine_mapping, _ = latlong_rect_to_affine_mapping(
                        lon_min=lon[0],
                        lon_max=lon[1],
                        lat_min=lat[0],
                        lat_max=lat[1],
                ...
                self._image.affine = affine_mapping_to_shader_param_value(affine_mapping)
...
def create_dfm_image_feature(
    ...
    carb.log_info(f"Getting image feature: {feature_name}")
    image_feature = ImageFeature.get_image_feature(feature_name)
    carb.log_info(f"Image feature: {image_feature}")
    # Register rgb images with timestamps
    if image_urls:
        carb.log_info(f"Updating RGB images: {image_urls}")
        image_feature.update_rgb_images(image_urls, image_timestamps)
        carb.log_info(f"Updated RGB images: {image_feature.image.sources}")
    ...
    carb.log_info(f"Updating date range: {image_timestamps}")
    image_feature.update_date_range(image_timestamps)
    carb.log_info(f"Updated date range")

    # Update metadata
    carb.log_info(f"Updating metadata: {meta_data}")
    image_feature.update_metadata(meta_data)
    carb.log_info(f"Updated metadata")

    carb.log_info(f"Updating transform: {meta_data}")
    image_feature.update_transform(meta_data, is_full_globe, adjust_offsets)
    carb.log_info(f"Updated transform")

    carb.log_info(f"Updating remapping: {meta_data}")
    image_feature.update_remapping(meta_data)
    carb.log_info(f"Updated remapping")

    # Set colormap
    if cmap:
        image_feature.image.colormap = cmap
    carb.log_info(f"Updated colormap")
    # Activate image
    carb.log_info(f"Activating image")
    image_feature.image.active = True
    carb.log_info(f"Activated image")

    # Rescale the time bar to the range of the currently created or updated layer
    if rescale_timeline:
        carb.log_info(f"Rescaling timeline")
