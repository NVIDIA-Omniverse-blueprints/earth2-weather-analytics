...
class Feature:
    ...
    def time_coverage(self, time_coverage: tuple[datetime, datetime]):
        ...
        if time_coverage is not None:
            ...
            self._property_change('time_coverage', (start, end))
        else:
    ...
    def _property_change(self, property_name: str, property_value: Any):
        ...
        def convert_datetime_objects(v):
            if isinstance(v, datetime.datetime):
                return v.isoformat()
            # we also have datetime tuples
        ...
        if change:
            old = convert_datetime_objects(cur_property_value)
            setattr(self, internal_name, property_value)
            # we can't send datetime objects over carb.events, so convert to string
            property_value = convert_datetime_objects(property_value)
