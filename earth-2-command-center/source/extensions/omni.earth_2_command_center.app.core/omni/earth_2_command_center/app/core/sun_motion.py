...
class SunMotion:
    ...
    def feature_property_changed(self, event):
        self._dirty = True
    ...
    def force_update(self, utc_time: datetime.datetime) -> bool:
        if feature := self._sun_feature_ref():
            ...
            feature.latitude = feature.latitude if (not feature.seasonal_motion or feature.follow_camera) else self._get_declination(utc_time)
            feature.longitude = feature.longitude if (not feature.diurnal_motion or feature.follow_camera) else (self._get_zenith(utc_time) - 180)
