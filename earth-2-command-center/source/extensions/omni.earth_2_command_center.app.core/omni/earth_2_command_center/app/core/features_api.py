...
class FeaturesAPI:
    ...
    def add_feature(self, feature: Feature, pos: Optional[int] = None):
        ...
        if feature.id == -1:
            carb.log_warn(f'Trying to add feature with invalid Feature Id: {feature}. Use features_apis create_*_feature() or create_feature(feature_type) instead.')
            feature._id = self._get_next_feature_id()
