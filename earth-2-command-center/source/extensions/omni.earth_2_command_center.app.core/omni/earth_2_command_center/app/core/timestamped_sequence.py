...
class SortedList:
    ...
    def insert(self, element: tuple[datetime, Any]):
        idx, closest_element = self.find_closest_smaller_equal(element[0])
        # if there is no element that comes before, just insert at the beginning
        if idx is None:
            self._list.insert(0, element)
        # exact match for timestamp -> replace
        elif closest_element[0] == element[0]:
            self._list[idx] = element
        # else, we insert after the found element
        else:
...
class GenericTimestampedSequence:
    ...
    def insert_multiple(self, entries):
        for e in entries:
            if not e[0].tzinfo:
                e = (e[0].replace(tzinfo=timezone.utc), e[1])
    ...
    def _time_event(self, event):
        ...
        if not self.loop or self.time_coverage is None:
            self._update_mapping(cur_utc_time)
        else:
            # check case where we are inside the range
            if self.time_coverage[0] <= cur_utc_time <= self.time_coverage[1]:
                self._update_mapping(cur_utc_time)
                return
            # do the time modulus operation
            period = self.time_coverage[1]-self.time_coverage[0]
            # early out if empty range
            if period == timedelta():
                self._update_mapping(self.time_coverage[0])
                return
            # we need to add (or subtract) period x times to cur_utc_time to
            # get it into our data range
            looped_utc_time = cur_utc_time + period*math.ceil((self.time_coverage[0]-cur_utc_time)/period)
            self._update_mapping(looped_utc_time)
    ...
    def _update_mapping(self, cur_utc_time: Optional[datetime] = None, force_update: bool = False):
        ...
        if not cur_utc_time.tzinfo:
            cur_utc_time = cur_utc_time.replace(tzinfo=timezone.utc)
...
class TimestampedSequence(GenericTimestampedSequence):
    ...
    def _on_update(self, cur_utc_time, target_idx, target_element):
        if not is_jpeg_path(target_element[1]):
            carb.log_error(f'Trying to load a non-jpeg through the jpeg decoder. Only jpegs are supported for image sequences: {target_element[1]}')
        else:
    ...
    def _on_update(self, cur_utc_time, target_idx, target_element):
        ...
        for i in range(len(self._tex_list)):
            if not is_jpeg_path(target_element[1][i]):
                carb.log_error(f'Trying to load a non-jpeg through the jpeg decoder. Only jpegs are supported for image sequences: {target_element[1][i]}')
            else:
...
class HPXTimestampedSequence(MosaicTimestampedSequence):
    def __init__(self):
        super().__init__(tileCount=12, urlPrefix="hpx")
