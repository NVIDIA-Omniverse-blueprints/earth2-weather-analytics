    ...
    def create_layer(stage:Usd.Stage,
        ...
        if feature.sources:
            ...
            if projection == 'latlong':
                ...
                tex_lookup_spec = shader_library.get_shader_spec('lookup_hpx_texture')
            else:
                raise ValueError(f'Unhandled Projection: \'{projection}\'')
            ...
            if projection == 'hpx':
                # rotate not handled yet
                tex_lookup_prim.GetInput('flip_u').Set(feature.flip_u)
                add_to_update_mapping(feature, 'flip_u', tex_lookup_prim.GetInput('flip_u'), simple_update_func)
                tex_lookup_prim.GetInput('flip_v').Set(feature.flip_v)
                add_to_update_mapping(feature, 'flip_v', tex_lookup_prim.GetInput('flip_v'), simple_update_func)

                if len(feature.sources) < 12:
                    raise ValueError("Image Feature with hpx projection but < 12 sources")
                for i in range(12):
                    tex_lookup_prim.GetInput(f'tex{i}').Set(feature.sources[i])
        ...
        if feature.alpha_sources:
            ...
            if projection == 'latlong':
                ...
                tex_lookup_spec = shader_library.get_shader_spec('lookup_hpx_texture_mono')
            else:
                raise ValueError(f'Unhandled Projection: \'{projection}\'')
            ...
            if projection == 'hpx':
                ...
                if len(feature.alpha_sources) < 12:
                    raise ValueError("Image Feature with hpx projection but < 12 sources")
                for i in range(12):
                    alpha_tex_lookup_prim.GetInput(f'tex{i}').Set(feature.alpha_sources[i])
        ...
        if bind_prim:
            ...
            def handling_recompilation(compilation_done_callback):
                ...
                sub = carb.eventdispatcher.get_eventdispatcher().observe_event(
                        # TODO: is this really dependent on the context? this method can't know for
                        # sure that the current context is the main one
                    event_name=omni.usd.get_context().stage_event_name(omni.usd.StageEventType.ASSETS_LOADED),
                    observer_name="[ext: omni.earth_2_command_center.app.shading] Shading::assets_loaded_callback",
