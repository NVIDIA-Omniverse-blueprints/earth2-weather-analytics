...
def write_cmweather_colormap(colormap_name, png_path):
    try:
        colormap = getattr(cmweather.cm, colormap_name)
    ...
    for i in range(num_rows):
        x = i/((num_rows-1) if num_rows>=2 else 1)
        val = colormap(x)
...
if __name__ == '__main__':
    ...
    write_cmweather_colormap(colormap_name, png_path)
