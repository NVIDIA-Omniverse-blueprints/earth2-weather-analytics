...
class SetupExtension(omni.ext.IExt):
    ...
    def on_startup(self, ext_id):
        ...
        # NOTE: check driver version due to driver bug
        # https://nvbugswb.nvidia.com/NvBugs5/SWBug.aspx?bugid=5131795
        import omni.kit.app
        ...
        def switch_renderer():
            ...
            if cur == "RaytracedLighting":
                settings.set_string("/rtx/rendermode", "RealTimePathTracing")
            elif cur == "RealTimePathTracing":
                settings.set_string("/rtx/rendermode", "PathTracing")
            else:
                settings.set_string("/rtx/rendermode", "RaytracedLighting")
            carb.log_warn(f'Rendermode set to: {settings.get("/rtx/rendermode")}')

        action_registry.register_action(self._ext_id, 'switch_renderer', switch_renderer, 'Switch Renderer', 'Toggle between Real-Time and RTPT2 and Path Tracing RTX Renderer')
    ...
    async def _delayed_layout(self):
        ...
        try:
            ...
            # ===============================================================================
            # Enable Frame Generation if it's available
            # ===============================================================================
            # has to be done delayed
            #if settings.get("/rtx-transient/hashed/995bcbda752ace40e33a4d131067d270"): # true if dlssg available
