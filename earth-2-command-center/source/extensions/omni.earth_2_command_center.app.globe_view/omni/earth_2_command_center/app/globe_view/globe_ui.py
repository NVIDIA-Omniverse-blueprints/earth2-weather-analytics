...
class GlobeUI:
    def __init__(self, ext_id: str, viewport_window):
        ...
        self.__visible = True
        self.__ctx = omni.usd.get_context()
    ...
    def add_tool(self, tool_description) -> int:
        cur_id = self._custom_tools_next_id
        self._custom_tools_next_id += 1
        self._custom_tools[cur_id] = tool_description
        # TODO: should this already trigger a rebuild of the ui?
        if self._toolbar_frame:
            self._toolbar_frame.rebuild()
        return cur_id
    ...
    def remove_tool(self, tool_id):
        if tool_id in self._custom_tools:
            ...
            carb.log_error(f'Attempted to delete a custom tool with id that isn\'t registered: {tool_id}')
        ...
        with self.__frame:
            with ui.ZStack(width=ui.Percent(100), height=ui.Percent(100)):
                if self.visible:
                    ...
                    self._toolbar_frame = ui.Frame(spacing=0)  # Top Left
                    ...
                    ui_elements_settings = [
                            ...
                            (f"/exts/{ext_name}/toolbar/visible", True, self._toolbar_frame)]
                    ...
                    self._toolbar_frame.set_build_fn(self.__build_toolbar_ui)
    ...
    # UI
    ############################################
    def __build_toolbar_ui(self):
        def build_button_group(name:str, clicked_fn: Callable, tooltip = '') -> ui.Button:
            with ui.HStack(height=30):
                ui.Spacer()
                button = ui.Button("", name=name,
                                   width=0, height=0,
                                   image_width=30, image_height=30,
                                   clicked_fn=clicked_fn, tooltip = tooltip)
                ui.Spacer()
            return button

        with ui.HStack(name="toolbar_stack", spacing=0, style=TOOLBAR_PANEL):
            ui.Spacer(width=30)
            # Panel
                ...
                with ui.ZStack(content_clipping=1, opaque_for_mouse_events=True):
                    ...
                    with ui.VStack(spacing=10):
                        ...
                        if self._custom_tools:
                            ui.Line(height=10, #alignment=ui.Alignment.H_CENTER,
                                    style={'color':_LIGHT})
                            for tool_id, desc in self._custom_tools.items():
                                button = build_button_group(desc.name,
                                                            partial(desc.callback, desc, self._toolbar_frame.rebuild),
                                                            tooltip=desc.tooltip)
                                button.image_url = desc.icon
    ...
    def __on_frame_size_changed(self) -> None:
        ...
        # self._toolbar_stack.visible = height > 240 and width >= 640  # example
