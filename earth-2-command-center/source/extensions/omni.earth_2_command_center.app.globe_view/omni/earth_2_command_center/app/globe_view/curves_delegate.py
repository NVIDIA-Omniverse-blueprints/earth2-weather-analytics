...
def get_projected_points(projection, points):
    if projection is None or str(projection).lower() == 'raw':
        return points
    else:
        # points need projection
        if projection.lower() in ['latlon', 'latlong', 'latlonalt', 'latlongalt']:
            num_points = len(points)
            proj_points = np.ndarray((num_points, 3))

            has_altitude = projection.lower() in ['latlonalt', 'latlongalt'] and points.shape[1] >= 3

            geo_converter = get_geo_converter()
            x,y,z = geo_converter.lonlatalt_to_xyz(
                    points[:,1],
                    points[:,0],
                    points[:,2] if has_altitude else 1)
            proj_points[:,0] = x
            proj_points[:,1] = y
            proj_points[:,2] = z
        else:
            raise RuntimeError(f'Unsupported projection: {projection}')
        return proj_points
    ...
    def __call__(self, event, globe_view):
        ...
        if id not in self._features_info:
            ...
            if feature.points is not None:
                prim.GetPointsAttr().Set(get_projected_points(feature.projection, feature.points))
        ...
        # handle property changes
            ...
            if property_name == 'active':
                ...
                curves_prim = feature_info['prim']
                if feature.points is not None:
                    curves_prim.GetPointsAttr().Set(get_projected_points(feature.projection, feature.points))
            ...
            # TODO: handle different ways to set width: constant, per curve, per vertex
                ...
                if feature.periodic:
                    curves_prim.GetWrapAttr().Set(UsdGeom.Tokens.periodic)
                else:
                    curves_prim.GetWrapAttr().Set(UsdGeom.Tokens.nonperiodic)
